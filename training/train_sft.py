...
def main() -> None:
    ...
    dataset = load_dataset("json", data_files=config.get("data_file", "data/sft/train.jsonl"), split="train", cache_dir=str(DATASETS_CACHE_DIR))
    ...
    training = SFTConfig(
        output_dir=config["output_dir"], num_train_epochs=config["num_train_epochs"],
        per_device_train_batch_size=config["per_device_train_batch_size"],
        gradient_accumulation_steps=config["gradient_accumulation_steps"],
        learning_rate=config["learning_rate"], logging_steps=config.get("logging_steps", 10),
        report_to=config.get("report_to", "none"), seed=config.get("seed", 42),
        bf16=config.get("bf16", False), max_length=config["max_seq_length"],
    )
