...
def main() -> None:
    ...
    command("evaluation/eval_refusal.py", "--model", args.base, "--model", args.sft, "--model", args.dpo, "--prompts-dir", str(args.prompts_dir), "--results-dir", str(results))
    command("evaluation/gate.py", str(results / f"{Path(args.dpo).name}.jsonl"))
